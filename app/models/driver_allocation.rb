class DriverAllocation < ActiveRecord::Base
  ...
  def self.notAvailableIdList(delivery_id)
    return DriverAllocation.where('(delivery_id=? AND completed=0 AND (active =-1 or active =0 or active =2  or active=3)) OR active=3 OR active=1', delivery_id).pluck(:driver_id);
  end

  def self.activeList
    return DriverAllocation.where('active=3 OR active=1').pluck(:driver_id);
  end

  def self.notAvailableIdListForReject(delivery_id)
    return DriverAllocation.where('(delivery_id=? AND completed=0 AND (active =0  or active=3)) OR active=3 OR active=1', delivery_id).pluck(:driver_id);
  end

  def self.no_response(delivery_id)
    return DriverAllocation.where('delivery_id=? AND attempt_count > 1', delivery_id).pluck(:driver_id)
  end
